...
AtoA <- simumorph(x = amp_pha_cov, m.space = amp_pha_mat, init = 1, target = 1, method = "AtoA", sim = sims, npts = npts, only.shapes = F, a = 0.2, e = 0.05, f = 100)

## Plot
png("../Figures/Fig_4.png", res = 50, height = 1500, width = 1500)
layout(mat)
for (i in 1:sims){
	plot(AtoA$Shapes[[i]], type = "l", lwd = 1.5, cex.main = 2, xlab = "", ylab = "", bty = "n", main = paste0("t = ",i))
	polygon(AtoA$Shapes[[i]], col = adjustcolor("seagreen", alpha = 0.85))
}
plot(unlist(AtoA$P.distances), type = "l", lwd = 3, cex.main = 2, cex.lab = 2.2, mgp = c(1.8,0.7,0), col = "seagreen3", main = "Procrustes distances", xlab = "Time", ylab = "Dist", bty = "n")
dev.off()
###################################################################################
...
AtoBa <- simumorph(x = amp_pha_cov, m.space = amp_pha_mat, init = 1, target = which(rownames(amp_pha_mat)=="G18_m_G18"), method = "AtoB", sim = sims, npts = npts, only.shapes = F, a = 0.5, e = 0.5, f = 100, max.attempts = 500)
